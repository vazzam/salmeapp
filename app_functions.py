...
- No utilices la palabras "autolisis" o "autolítico"
 ...
 # --- Gestión de Estado con Claves Únicas ---
    audio_key = f"audio_data_{nota}"
    last_id_key = f"last_id_{nota}"
    ...
    if last_id_key not in st.session_state:
        st.session_state[last_id_key] = 0 # Usamos 0 como valor inicial para la id
    ...
    with col1:
        audio_result = mic_recorder(
            ...
            format="wav", # El formato wav es compatible directamente con st.audio
    ...
    # --- Lógica Principal usando la 'id' para detectar nuevo audio ---
    if audio_result and audio_result['id'] > st.session_state[last_id_key]:
        st.session_state[last_id_key] = audio_result['id']
        st.session_state[audio_key] = audio_result['bytes']
        st.session_state[transcription_key] = None # Limpiar transcripción anterior
        st.rerun()

    # --- Mostrar Reproductor de Audio y Botones de Acción ---
    if st.session_state[audio_key] is not None:
        ...
        if st.button("🔮 Transcribir y Resumir", disabled=not can_transcribe, type="primary"):
            st.session_state[processing_key] = True
            with st.spinner("Transcribiendo..."):
                # Aquí llamas a tu función real de transcripción y resumen
                # Por ejemplo:
                # transcription = tu_funcion_whisper(st.session_state[audio_key])
                # summarized_text = tu_funcion_resumen(transcription, nota)
                # st.session_state[transcription_key] = summarized_text
                
                # Usando la función de ejemplo:
                st.session_state[transcription_key] = transcribe_audio(st.session_state[audio_key])
