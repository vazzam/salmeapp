...
def split_audio(audio_path: str, segment_duration_ms: int = 600000): # Segmentos de 10 minutos
    """Divide el audio desde un archivo en fragmentos menores."""
    try:
        # st.info("Segmentando el audio para su procesamiento...")
        ...
        for start_ms in range(0, duration_ms, segment_duration_ms):
            ...
            # Exportar cada segmento a un BytesIO para la transcripción
            segment_io = io.BytesIO()
            segment.export(segment_io, format="wav")
            segment_io.seek(0)
            segments.append(segment_io)
            
        # st.success(f"Audio dividido en {len(segments)} segmento(s).")
        return segments
    except Exception as e:
        st.error(f"Error al segmentar el audio: {str(e)}")
        return None

def transcribe_audio(audio_path, nota):
    """Transcribe el audio segmentado mostrando una barra de progreso."""
    progress_bar = None # Inicializar por si ocurre un error antes de su creación
    try:
        ...
        # 1. Inicializar la barra de progreso
        progress_bar = st.progress(0, text="Iniciando transcripción...")

        # 2. Transcribir cada segmento, actualizando la barra de progreso
        for i, segment_io in enumerate(segments):
            # Actualizar el texto y el valor de la barra
            progress_text = f"Segmento {i + 1} de {total_segments}..."
            progress_value = (i + 1) / total_segments
            progress_bar.progress(progress_value, text=progress_text)
            
            segment_io.name = f"segment_{i}.wav"
        ...
        # 3. Limpiar la barra de progreso al finalizar
        progress_bar.empty()

        if full_transcription:
            try:
                summarized = resumen_transcripcion(full_transcription.strip(), nota)
                summarized2 = resumen_transcripcion2(full_transcription.strip(), nota)
        ...
        # Si ocurre un error, asegurarse de que la barra de progreso se elimine
        if progress_bar:
            progress_bar.empty()
...
def audio_recorder_transcriber(nota: str):
    """Función reutilizable para grabar, guardar, transcribir y resumir audio."""
...
# Interfaz
    ...
    with col1:
        ...
        if audio_value:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".webm") as temp_audio_file:
                temp_audio_file.write(audio_value['bytes'])
                st.session_state.audio_path = temp_audio_file.name

            st.audio(st.session_state.audio_path)
        # st.success("El audio está listo para ser reproducido o transcrito.")
    with col2:
        if st.button("🔮 Resumir", disabled=not st.session_state.audio_path):
            # El spinner ahora envuelve todo el proceso, y la barra de progreso
            # aparecerá dentro de la función transcribe_audio.
            with st.spinner("Preparando todo..."):
                transcripcion = transcribe_audio(st.session_state.audio_path, nota)
                if transcripcion:
    ...
    if st.session_state.transcripcion:
        ...
        # st.markdown(st.session_state.transcripcion)
        if st.button("Limpiar para nueva nota"):
            st.session_state.transcripcion = ""
            st.rerun()
