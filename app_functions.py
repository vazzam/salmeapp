...
def audio_recorder_transcriber(nota: str):
    """
    Función reutilizable para grabar, guardar en disco y transcribir audio.
    """
    # --- Funciones internas (sin cambios) ---

    def transcribe_audio(audio_file): # <-- MODIFICADO: Ahora recibe un archivo
        """Transcribe el audio."""
        try:
            response = client.audio.transcriptions.create(
                model="openai/whisper-large-v3-turbo",
                file=audio_file, # <-- MODIFICADO: Se pasa directamente el archivo
                language="es"
            )
            
            if response.text:
                st.success("Transcripción completa exitosa")
                # Intenta obtener resúmenes de ambos modelos
                try:
                    summarized = resumen_transcripcion(response.text, nota)
                    summarized2 = resumen_transcripcion2(response.text, nota)
                    return f"{summarized}\n\n--- VERSION 2: --------»»\n\n{summarized2}"
                except Exception as e:
                    st.warning(f"No se pudo obtener el resumen de Gemini, mostrando solo la versión 2. Error: {e}")
                    summarized2 = resumen_transcripcion2(response.text, nota)
                    return summarized2
            return None
        except Exception as e:
  ...
  - No utilices la palabras "autolisis" o "autolítico"
    ...
    # --- Inicialización del Estado (MODIFICADO) ---
    if "audio_filepath" not in st.session_state:
        st.session_state["audio_filepath"] = None # <-- NUEVO: Guardará la ruta del archivo
    if "transcripcion" not in st.session_state:
        st.session_state["transcripcion"] = ""
    # Se elimina la necesidad de st.session_state["audio_data"] y st.session_state["is_recording"]

    # --- Interfaz de Grabación ---
    st.text('')
    audio_value = mic_recorder(
        start_prompt="▶️ Iniciar",
        stop_prompt="⏹️ Detener",
        just_once=False,
        use_container_width=True,
        format="wav", # <-- MODIFICADO: Es mejor especificar WAV si la API lo prefiere
        key='recorder'
    )
    
    # --- Lógica de Guardado (NUEVA) ---
    if audio_value:
        st.info("Grabación recibida, guardando archivo...")
        # Define una carpeta temporal para los audios
        audio_dir = "temp_audio"
        if not os.path.exists(audio_dir):
            os.makedirs(audio_dir)
        
        # Guarda los bytes del audio en un archivo .wav en el servidor
        filepath = os.path.join(audio_dir, "grabacion_actual.wav")
        with open(filepath, "wb") as f:
            f.write(audio_value['bytes'])
        
        # Guarda la ruta del archivo en el estado de la sesión para usarla después
        st.session_state.audio_filepath = filepath
        st.session_state.transcripcion = "" # Limpia la transcripción anterior
        st.rerun() # Fuerza un refresco de la app para mostrar el reproductor

    # --- Interfaz de Reproducción y Transcripción (MODIFICADA) ---
    if st.session_state.audio_filepath:
        st.success("Grabación guardada. Lista para transcribir.")
        
        # Muestra el reproductor de audio usando la ruta del archivo
        ...
        if st.button("🔮 Transcribir y Resumir", use_container_width=True):
            with st.spinner("Transcribiendo y resumiendo el audio... ⏳"):
                try:
                    # Lee el archivo desde el disco y lo pasa a la función
                    with open(st.session_state.audio_filepath, "rb") as audio_file:
                        transcripcion = transcribe_audio(audio_file)
                    
                    if transcripcion:
                        st.session_state["transcripcion"] = transcripcion
                        # Limpia la ruta del archivo para que la interfaz se resetee
                        st.session_state.audio_filepath = None 
                        st.rerun() # Refresca para mostrar la transcripción

                except Exception as e:
                    st.error(f"Ocurrió un error en el proceso: {e}")

    # Devuelve la transcripción final para ser mostrada en tu página principal
